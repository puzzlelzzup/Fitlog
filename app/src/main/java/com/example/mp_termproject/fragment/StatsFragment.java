...
public class StatsFragment extends Fragment {
    ...
    private void setupGoalSeekBars() {
        ...
        seekbarTargetWeight.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar seekBar) {
                ...
                userRef.child("stats").child("target_weight").setValue(weight)
                        .addOnSuccessListener(aVoid ->
                                Toast.makeText(requireContext(), "목표 체중 저장됨: " + weight + "kg", Toast.LENGTH_SHORT).show())
        ...
        seekbarWorkoutGoalDays.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar seekBar) {
                ...
                userRef.child("stats").child("workout_goal_days").setValue(days)
                        .addOnSuccessListener(aVoid ->
                                Toast.makeText(requireContext(), "운동 목표 저장됨: " + days + "일", Toast.LENGTH_SHORT).show())
                        .addOnFailureListener(e ->
                                Toast.makeText(requireContext(), "저장 실패: " + e.getMessage(), Toast.LENGTH_SHORT).show());
            }
        });
        ...
        seekbarCalorieIntakeGoal.setOnSeekBarChangeListener(new SeekBar.OnSeekBarChangeListener() {
            ...
            @Override public void onStopTrackingTouch(SeekBar seekBar) {
                ...
                if (userRef != null) {
                    userRef.child("stats").child("calorie_intake_goal").setValue(finalProgress)
                            .addOnSuccessListener(aVoid ->
                                    Toast.makeText(requireContext(), "일일 칼로리 목표 저장됨: " + finalProgress + "kcal", Toast.LENGTH_SHORT).show())
                            .addOnFailureListener(e ->
                                    Toast.makeText(requireContext(), "저장 실패: " + e.getMessage(), Toast.LENGTH_SHORT).show());
                } else {
                    Toast.makeText(requireContext(), "로그인 후에 설정할 수 있습니다.", Toast.LENGTH_SHORT).show();
                }
            }
        });
    ...
    private void loadAndDisplayData(String period) {
        ...
        userRef.child("stats").child("workout_goal_days").get().addOnSuccessListener(snapshot -> {
            if (snapshot.exists()) {
                int days = snapshot.getValue(Integer.class);
                seekbarWorkoutGoalDays.setProgress(days);
                tvWorkoutGoalDaysValue.setText(days + "일");
            }
        });

        // 목표 체중
        userRef.child("stats").child("target_weight").get().addOnSuccessListener(snapshot -> {
            if (snapshot.exists()) {
                int weight = snapshot.getValue(Integer.class);
                seekbarTargetWeight.setProgress(weight);
                tvTargetWeightValue.setText(weight + "kg");
            }
        });
        userRef.child("stats").child("calorie_intake_goal").get()
                .addOnSuccessListener(snapshot -> {
                    if (snapshot.exists()) {
                        Integer calorie = snapshot.getValue(Integer.class);
                        if (calorie != null) {
                            seekbarCalorieIntakeGoal.setProgress(calorie);
                            tvCalorieIntakeGoalValue.setText(calorie + "kcal");
                        }
                    }
                });
