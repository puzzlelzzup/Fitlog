...
public class GridAdapter extends ArrayAdapter {
    ...
    public View getView(int position, final View convertView, final ViewGroup parent) {
        ...
        // sunday and saturday different set the red and blue color
        Calendar tempCal = Calendar.getInstance();
        tempCal.setTime(mDate);
        int dayOfWeek = tempCal.get(Calendar.DAY_OF_WEEK);
        if (dayOfWeek == Calendar.SUNDAY) {
            cellNumber.setTextColor(Color.parseColor("#FF4D4D"));
        } else if (dayOfWeek == Calendar.SATURDAY) {
            cellNumber.setTextColor(Color.parseColor("#2196F3"));
        }
